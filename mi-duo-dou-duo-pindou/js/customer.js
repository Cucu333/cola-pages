...
function renderCustomerView() {
  ...
  container.innerHTML = '<section class="customer-seat-info customer-seat-card"><div class="customer-card-top"><div class="customer-package">' + (pkg ? pkg.name : '当前套餐') + '</div><span class="customer-status-badge ' + statusClass + '">' + (state.arrivalType || '到店') + ' · ' + statusValue + ' · ¥' + Number(pkg?.price || 0).toFixed(1) + '</span></div><div class="customer-animals">' + animal + '</div><div class="customer-seat-number">' + currentSeatId + '</div><div class="customer-status-text">' + (state.status === 'paused' ? '暂停' : overtime ? '超时' : '占用') + '</div><div class="customer-timer" id="customer-timer-display">' + (remaining === Infinity ? '不限时' : formatTime(remaining)) + '</div><div class="customer-times"><span>开台时间：' + new Date(state.startTime).toLocaleTimeString() + '</span><span>' + endLabel + '</span></div><div class="customer-related">关联座位：' + (state.orderSeatIds || [currentSeatId]).join('+') + '</div>' + (state.status === 'paused' ? '<div class="request-badge">店员已暂停计时</div>' : '') + iron + (state.workStatus === 'unfinished' ? '<button class="work-dot customer-work-dot" title="作品未完成，点击继续" onclick="toggleWorkForm()">●</button>' : '<button class="work-dot customer-work-dot work-dot-empty" title="保存未完成作品" onclick="toggleWorkForm()">●</button>') + '<div class="customer-actions">' + extraButton + '<button class="customer-btn secondary" onclick="openIronForm()">申请熨烫</button></div>' + (customerExtraFormOpen ? renderExtraForm() : '') + (customerIronFormOpen ? renderIronForm() : '') + (customerWorkFormOpen ? renderWorkForm() : '') + '</section>';
}
...
function renderIronForm() {
  ...
  return '<div class="inline-form" id="iron-form"><h3>申请熨烫</h3><div class="customer-choice-label">熨烫方式</div><div class="customer-iron-tags">' + modes.map((mode, i) => '<button type="button" class="status-tag ' + (i === 0 ? 'selected' : '') + '" onclick="selectCustomerIronMode(\'' + mode + '\', this)">' + mode + '</button>').join('') + '</div>' + renderIronAccessories() + '<label class="iron-upload-label">上传拼豆图片（可选）<input id="iron-image" type="file" accept="image/*" capture="environment" onchange="showIronFile(this)"><small id="iron-file-name">点击选择图片，选好后会显示文件名</small></label><button type="button" class="customer-btn" onclick="submitIronRequest()">确认提交熨烫</button></div>';
}
...
function openIronForm() { customerExtraFormOpen = false; customerWorkFormOpen = false; selectedCustomerIronAccessories = []; pendingCustomerIronMode = getIronModes()[0] || '普通熨烫'; customerIronFormOpen = true; renderCustomerView(); }
